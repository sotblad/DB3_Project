...
csv_reader = csv.reader(open('countries/joined.csv'), delimiter=',')
line_count = 0
years = {}
...
#            for i in years.keys():
#                if(row[years[i]] != ""):
#                    sqlq = f'INSERT INTO `{i}`(Country, Indicator, Value) VALUES ("{row[0]}", "{row[3]}", {row[years[i]]})'
#                    cursor.execute(sqlq)
#
            line_count += 1
